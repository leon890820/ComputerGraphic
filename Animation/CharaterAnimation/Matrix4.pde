...
class MatrixX {
    ...
    public void setColumn(int index , float[] d){
        if(index >= cols) return;
        if(d.length != rows) return;
        for(int i = 0; i < rows; i++){
            w[index][i] = d[i];
        }        
    }
    
    public void setRow(int index , float[] d){
        if(index >= rows) return;
        if(d.length != cols) return;
        for(int i = 0; i < cols; i++){
            w[i][index] = d[i];
        }        
    }

    public MatrixX add(MatrixX a) {
        if (a.cols != cols || a.rows != rows) return null;
        MatrixX result = new MatrixX(a.cols, a.rows);
    ...
    MatrixX scale(float n) {
        MatrixX result = new MatrixX(cols, rows);
        for (int i = 0; i < cols; i++) {
    ...
    MatrixX mult(MatrixX a) {
        MatrixX result = new MatrixX(cols, a.rows);
    ...
    public VectorX mult(VectorX v) {
        VectorX result = new VectorX(cols);
        if (rows != v.s) return null;
        for (int i = 0; i < cols; i++) {
            float sum = 0.0;
            for (int j = 0; j < rows; j++) {                
                sum += w[i][j] * v.v[j];
    ...
    MatrixX transpose() {
        MatrixX result=new MatrixX(rows, cols);
        for (int i = 0; i < cols; i++) {
            for (int j = 0; j < rows; j++) {
                result.w[j][i] = w[i][j];
            }
