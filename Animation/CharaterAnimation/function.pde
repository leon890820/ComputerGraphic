...
public float[] stringToFloat(String[] s, ArrayList<Joint> joint , IntList order){
    float[] result = new float[s.length];
    int count = 0;
    for(int i = 0; i < joint.size(); i++){
        int channel = joint.get(order.get(i)).channel;
        if(channel == -1) continue;
        for(int j = 0; j < joint.get(order.get(i)).channel_size ; j++){            
             result[count] = float(s[channel + j]);
             count ++ ;
        }
    }
    return result;
