...
JumpFloodMaterial jumpFloodMaterial;
JumpFloodPassMaterial jumpFloodPassMaterial;
QuadMaterial quadMaterial;

Quad JFARenderer;
Quad JFARendererPass;
Quad JFARendererFinal;
...
void setup() {
    ...
    texture = new Texture("Textures/JFA6.png");
    jumpFloodPassMaterial.setTexture(texture);
    //firstJFATarget.bindFrameBuffer();    
    //JFARenderer.run();
    
    ////JFATarget.bindFrameBuffer();    
    ////JFARendererPass.run();
 
    //gl3.glBindFramebuffer(gl3.GL_FRAMEBUFFER , 0);
    background(0);
    JFARendererFinal.run();
...
void draw() {
    ...
    if(frameCount == 2) {
        saveFrame("data/Textures/JFA7.png");
        noLoop();
    }
...
void setGameObject() {
    JFARenderer = new Quad(jumpFloodMaterial);
    JFARendererPass = new Quad(jumpFloodPassMaterial);
    JFARendererFinal = new Quad(quadMaterial);
}

void setMaterial() {  
    
    firstJFATarget = new FBO(width, height, 1);
    JFATarget = new FBO(width,height,1);
    
    Texture JFAOrigin = new Texture("Textures/JFA.png");
    JFAOrigin.setSamplingMode(2).setWrapMode(CLAMP);
    
    jumpFloodMaterial = new JumpFloodMaterial("Shaders/JumpFlood.frag" , "Shaders/JumpFlood.vert");
    jumpFloodMaterial.setTexture(JFAOrigin);
    
    jumpFloodPassMaterial = new JumpFloodPassMaterial("Shaders/JumpFloodPass.frag" , "Shaders/JumpFloodPass.vert");
    jumpFloodPassMaterial.setTexture(firstJFATarget.tex[0]).setTexture2(JFATarget.tex[0]).setStep(step).setFirst(true);
    
    quadMaterial = new QuadMaterial("Shaders/quad.frag" , "Shaders/quad.vert");
    quadMaterial.setTexture(JFATarget.tex[0]);
