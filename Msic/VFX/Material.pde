...
public class PortalMaterial extends Material {
    ...
    public void initGUI(){
        gui = new GUIManager();    
        gui.addSlider(this,strength,"strength",new Vector3(20,20,0),new Vector3(0,10,0));
    }
    
    public PortalMaterial setNoiseTexture(Texture t) {
        worleyNoiseTex = t;
        return this;
    }
    ...
    public void run(GameObject go) {
        setGameobject(go);
        setMatrix4ToUniform("MVP", gameobject.MVP().transposed());
        setFloatToUniform("strength", strength);
        setFloatToUniform("offset", -(float)frameCount/(float)20);
        shader.set("worleyNoiseTex", worleyNoiseTex.img);
    }
