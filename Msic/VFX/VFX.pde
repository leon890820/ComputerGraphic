...
void setup() {
    ...
    worleyNoiseGenerator = new WorleyNoiseGenerator(200,200,3);
...
void setGameObject() {
    portal = new QuadObject(portalMaterial);
    //dragon.setScale(50, 50, 50);    
    floor = new QuadObject(floorMaterial);
    //floor.setEular(90 , 0 , 0).setPosition(0,-2,0);


}

void setMaterial() {  
    portalMaterial = new PortalMaterial("Shaders/portal.frag", "Shaders/portal.vert");
    portalMaterial.initGUI();
    
    worleyNoiseTexture = new Texture(1,1);
    worleyNoiseTexture.setTexture(worleyNoiseGenerator.noise);    
    portalMaterial.setNoiseTexture(worleyNoiseTexture);
...
public void cameraSetting() {
    ...
    main_camera.setPosition(0.0, 0.0, 2.0).setEular(-0.0, 0.0, 0.0);
