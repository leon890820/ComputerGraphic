class Capsule extends Object{
  
  PImage bump=null;//loadImage("Textures/Brick_Normal.JPG");
  
  Capsule(){
    mesh=new Mesh("capsule.obj");
    texture=null;//new Texture("Brick_Diffuse.JPG");
  }
  ...
  color frag(float[] coord,Frag f){
    ...
    Vector3 tangent_normal=bump==null? new Vector3(1,1,1) : getTextureColor(bump,new Vector3(u.z,u.w,0));
    
    tangent_normal = new Vector3(tangent_normal.x-0.5,tangent_normal.y-0.5,tangent_normal.z-0.5);
    tangent_normal.x*=2;
    tangent_normal.y*=2;
    ...
    //tangent_normal.z=sqrt(1-max(0,tangent_normal.x*tangent_normal.x+tangent_normal.y*tangent_normal.y));
    ...
    Vector3 dif = light.light_color.product(albedo).mult(max(0,Vector3.dot(tangent_light_dir,tangent_normal)));
