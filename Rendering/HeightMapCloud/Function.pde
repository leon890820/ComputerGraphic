...
public Vector3 clamp(Vector3 v, float a, float b) {
    return new Vector3(clamp(v.x,a,b),clamp(v.y,a,b),clamp(v.z,a,b));
}
...
public Vector3 pow(Vector3 v,float a){
    return new Vector3(pow(v.x,a),pow(v.y,a),pow(v.z,a));
}
...
public Vector3 lerp(Vector3 a,Vector3 b,float w){
    return a.mult(1-w).add(b.mult(w));
}

public int getIndex(Vector3 v){  
    v = clamp(v,-1.0,1.0);
    int x = int(map(v.x,-1,1,0,width-1));
    int y = int(map(v.y,-1,1,0,height-1));
...
public Vector3[] LocalBasis(Vector3 n) {
  ...
  float b = n.x * n.y * a;
  Vector3 b1 = new Vector3(1.0 + sign_ * n.x * n.x * a, sign_ * b, -sign_ * n.x);
  Vector3 b2 = new Vector3(b, (sign_ + n.y * n.y * a), n.y);
  return new Vector3[]{b1,b2};
  //float rotX = -PI/2 + acos(Clamp(n.z(), -1.0, 1.0));
  //float rotY = -PI/2+atan2(n.y(), n.x());
  //return Matrix4.RotX(rotX).mult(Matrix4.RotY(rotY));
  
  
}

Object[] SampleHemisphereCos(){
        Vector3 uv = new Vector3(random(1),random(1),random(1));
        float z = uv.x;
