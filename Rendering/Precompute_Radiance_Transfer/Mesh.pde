class Mesh {
    ...
    void addFace(int a, int at, int b, int bt, int c, int ct) {
        ...
        bt-=1;
        ct-=1;
        if(a<0) a+=verts.size()+1;
        if(b<0) b+=verts.size()+1;
        if(c<0) c+=verts.size()+1;
        if(at<0) at+=uvs.size()+1;
        if(bt<0) bt+=uvs.size()+1;
        if(ct<0) ct+=uvs.size()+1;
        Vector3[] vs={verts.get(a), verts.get(b), verts.get(c)};
        Vector3[] normal = new Vector3[3];
        if (normals.size()<=0) {
            for (int i=0; i<3; i+=1) {
                normal[i]=Vector3.cross(Vector3.sub(vs[(i+1)%3], vs[i]), Vector3.sub(vs[(i+2)%3], vs[i])).unit_vector();
            }
        } else {
            normal[0]=normals.get(a);
            normal[1]=normals.get(b);
            normal[2]=normals.get(c);
        }
        if(at>uvs.size()){
            ...
            Vector3[] us={new Vector3(),new Vector3(),new Vector3()};
            int[] triangle={a, b, c};
            triangles.add(new Triangle(vs, us, normal, triangle));
        }
    }
