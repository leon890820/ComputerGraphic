...
void mouseDragged() {
    ...
    px = map(px, -1, 1, -0.005, 0.005);
    py = map(py, -1, 1, -0.005, 0.005);
    church.setRotXandY(px, -py);
}
...
void setSkyBox(SkyBox skybox) {
    float z = width / (2 * tan(GH_FOV*PI/180));
    for (int i=0; i<width; i+=1) {
        for (int j=0; j<height; j+=1) {
            float x = map(i, 0, width-1, -width/2, width/2);
            float y = map(j, 0, height-1, height/2, -height/2);
            int index = j * width + i;
            colorBuffer[index] = skybox.getLightColor(new Vector3(x, y, z));
        }
    }
}

void setSkyBoxSH(SkyBox skybox){
    float z = width / (2 * tan(GH_FOV*PI/180));
    for (int i=0; i<width; i+=1) {
        for (int j=0; j<height; j+=1) {
            float x = map(i, 0, width-1, -width/2, width/2);
            float y = map(j, 0, height-1, height/2, -height/2);
            int index = j * width + i;
            Vector3 dir = new Vector3(x,y,z).unit_vector();
            Vector3 col = new Vector3();
            for(int l=0;l<skybox.SHOrder;l+=1){
                for(int m=-l;m<=l;m+=1){
                    int id = l*(l+1)+m;
                    col = col.add(skybox.shCoeffs[id].mult(SphereHarmonic.EvalSH(l,m,dir)));
                }
            }
