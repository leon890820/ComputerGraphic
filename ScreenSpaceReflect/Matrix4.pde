static class Matrix4{
  ...
  void makeGRM(Vector3 v1,Vector3 v2,Vector3 v3) {
...
static class Matrix{
    ...
    public void setRow(int c,float[] v){
        if(v.length!=r) {
            println("DisMatch Row");
            return;
        }
        m[c] = v;
    }
    ...
    public Matrix mult(Matrix a){
        ...
        Matrix result = new Matrix(c,a.r);
        for(int i=0;i<c;i+=1){
            for(int j=0;j<a.r;j+=1){
                float sum = 0.0;
                for(int k = 0;k < r;k+=1){
                    sum += m[i][k]*a.m[k][j];
                }
