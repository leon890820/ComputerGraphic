...
public Neighbor[] unionNeighbor(Neighbor[] n1 , Neighbor[] n2){
    ...
    for(int i = 0; i < n2.length; i++){
        ...
        for(int j = 0; j < result.size(); j++){
            if(result.get(j).equals(n2[i])){
                flag = true;
                break;
            }
        }
        if(!flag) result.add(n2[i]);
        
    }
    Neighbor[] n = new Neighbor[result.size()];
    result.toArray(n);
    return n;
    
}

int[] dirIndex = new int[]{0, 2, 1, 3}; 

public Neighbor[] getNeiborDir(Neighbor neighbor, int dir){
    TileState ts = neighbor.tile;
    int rotation = neighbor.rotation;
    int index = dir;
    if(dir < 4) {
        int b = dir == 1 ? 2 : (dir == 2 ? 1 : dir);
        int is = (b - rotation) % 4;        
        index = dirIndex[is < 0 ? is + 4 : is];
       
    }
    switch(ts){
        case Down:
            return rotateNeighbor(TileData.DownNeighbor[index] ,rotation);
        case Line:
            return rotateNeighbor(TileData.LineNeighbor[index] , rotation);
        case Turn:
            return rotateNeighbor(TileData.TurnNeighbor[index] , rotation);
        case Up:
            return rotateNeighbor(TileData.UpNeighbor[index] , rotation);
        case X:
            return rotateNeighbor(TileData.XNeighbor[index] ,rotation);
        case Empty:
            return rotateNeighbor(TileData.EmptyNeighbor[index] , rotation);
    }
    return null;
}

Neighbor[] rotateNeighbor(Neighbor[] neighbor, int rotation){
    Neighbor[] result = new Neighbor[neighbor.length];
    for(int i = 0; i < neighbor.length; i++){
        result[i] = new Neighbor(neighbor[i].tile , (neighbor[i].rotation + rotation) % 4);
    }
...
public Neighbor randomChoseNeighborWithWeight(ArrayList<Neighbor> list){
    ArrayList<Neighbor> pool = new ArrayList<Neighbor>();
    
    for(Neighbor n : list){
        for(int i = 0; i < weight[getTileNumber(n.tile)]; i++){
            pool.add(n);
        }
    }
...
public boolean outOfBoundary(Vector3 position){
    if(position.x < 0 || position.x >= size || position.y < 0 || position.y >= size || position.z < 0 || position.z >= size) return true;
    return false;
}
